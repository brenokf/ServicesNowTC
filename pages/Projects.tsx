...
const Projects: React.FC<ProjectsProps> = ({ user }) => {
  ...
  const ProjectDetailModal = () => {
    if (!isViewModalOpen || !selectedProject) return null;
      ...
      <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 sm:p-6 lg:p-8 overflow-hidden">
        ...
        <div className="relative z-[10001] w-full max-w-6xl max-h-[92vh] flex flex-col bg-[#FDFDFF] rounded-[3rem] shadow-[0_32px_128px_-16px_rgba(0,0,0,0.4)] animate-scaleUp overflow-hidden border border-white/20">
            ...
            {activeTab === 'Projeto' && (
              <div className="animate-fadeIn space-y-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  ...
                  <MetaItem label="Software Version" value="One UI 6.1 (Android 14)" />
                  <MetaItem label="Homologação" value="Completa / Tier 1" />
                  <MetaItem label="Data Início" value="12/10/2024" />
                  <MetaItem label="Entregas" value="Lote 01, Lote 02" />
                  <MetaItem label="Caderno ID" value="CAD-2024-UX-V3" />
                  <MetaItem label="Status Final" value="Aguardando Workshop" />
                </div>
                ...
                <div className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100">
                   <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4">Notas Estratégicas</h4>
          ...
          <div className="px-10 py-8 border-t border-slate-100 bg-white flex justify-end gap-4 shrink-0">
             <button onClick={handleCloseModals} className="px-10 py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-slate-200">Finalizar Visualização</button>
          </div>
        </div>
      </div>,
      document.body
  ...
  return (
    <div className="p-4 lg:p-8 animate-fadeIn">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10">
        <div><h2 className="text-3xl font-black text-slate-900 tracking-tighter">Project Allocation Hub</h2><p className="text-slate-500 text-sm mt-1">Gerencie os nós de homologação globais.</p></div>
        {isAdmin && <button onClick={() => handleOpenNewModal()} className="bg-indigo-600 text-white px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all flex items-center gap-2"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 4v16m8-8H4" /></svg>Novo Projeto</button>}
      </div>
      
      <div className="bg-white p-5 rounded-[2rem] border border-slate-200 shadow-sm mb-8 flex items-center gap-4">
        <div className="relative flex-1"><svg className="w-5 h-5 absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg><input type="text" placeholder="Pesquisar projetos ativos..." className="w-full pl-12 pr-6 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-semibold focus:bg-white transition-all outline-none" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div>
      </div>
